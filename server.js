...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const parsedDob = dob || null;

    // ── Look for an existing resident record uploaded from the device ─────────
    // Priority 1: name + dob + barangay (strongest match)
    // Priority 2: name + age + barangay (fallback when no DOB on device)
    let existingResident = null;

    if (parsedDob && barangay && name) {
      const dobCheck = await pool.query(
        `SELECT id, username FROM residents
         WHERE LOWER(TRIM(name)) = LOWER(TRIM($1))
           ...
           AND dob::date = $3::date
         LIMIT 1`,
        [name.trim(), barangay, parsedDob]
      );
      existingResident = dobCheck.rows[0] || null;
    }

    if (!existingResident && parsedAge && barangay && name) {
      const ageCheck = await pool.query(
        `SELECT id, username FROM residents
         WHERE LOWER(TRIM(name)) = LOWER(TRIM($1))
           ...
           AND age = $3
         LIMIT 1`,
        [name.trim(), barangay, parsedAge]
      );
      existingResident = ageCheck.rows[0] || null;
    }

    if (existingResident) {
      // ── Device-uploaded record found — link this new account to it ───────────
      const oldUsername = existingResident.username;

      // Create the new user login account
      await pool.query(
        `INSERT INTO users(username, password, role, name, email)
         VALUES($1, $2, 'resident', $3, $4)`,
        [username, hashedPw, name, email || null]
      );

      // Re-assign the existing resident row to the new username
      // Only overwrite blank fields — keep whatever the device already recorded
      await pool.query(
      ...
      // Remove the old placeholder user row if it was auto-generated by device upload
      // (auto-generated usernames follow the pattern: name_timestamp e.g. juan_dela_cruz_1718000000)
      if (oldUsername && oldUsername !== username && /^.+_\d{8,}$/.test(oldUsername)) {
        await pool.query(`DELETE FROM users WHERE username=$1`, [oldUsername]);
      }

      const linked = await pool.query(
        `SELECT * FROM residents WHERE username=$1`, [username]
      );
      return res.json({ message: 'Account linked to existing resident profile', user: linked.rows[0] });
    }

    // ── No existing device record — normal fresh registration ─────────────────
    await pool.query(
      `INSERT INTO users(username, password, role, name, email) VALUES($1,$2,'resident',$3,$4)`,
      [username, hashedPw, name, email || null]
    );
...
app.post('/api/update-account', async (req, res) => {
  try {
    ...
    // Build update query
    const emailVal = req.body.email || null;
    if (newPassword && profilePicUrl) {
      const hashedPw = await bcrypt.hash(newPassword, 10);
      await pool.query(
        'UPDATE users SET name=$1, password=$2, profile_pic=$3, email=$4 WHERE username=$5',
        [name, hashedPw, profilePicUrl, emailVal, username]
      );
    } else if (newPassword) {
      const hashedPw = await bcrypt.hash(newPassword, 10);
      await pool.query(
        'UPDATE users SET name=$1, password=$2, email=$3 WHERE username=$4',
        [name, hashedPw, emailVal, username]
      );
    } else if (profilePicUrl) {
      await pool.query(
        'UPDATE users SET name=$1, profile_pic=$2, email=$3 WHERE username=$4',
        [name, profilePicUrl, emailVal, username]
      );
    } else {
      await pool.query(
        'UPDATE users SET name=$1, email=$2 WHERE username=$3',
        [name, emailVal, username]
      );
    }
